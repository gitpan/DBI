 ...
 * contents of DBI.xs. Don't edit this file, edit DBI.xs instead.
 *
...
XS(XS_DBI__setup_handle)
{
    dXSARGS;
    if (items < 3 || items > 4)
	croak("Usage: DBI::_setup_handle(sv, imp_class, parent, imp_datasv=Nullsv)");
    {
	SV *	sv = ST(0);
	char *	imp_class = (char *)SvPV(ST(1),na);
	SV *	parent = ST(2);
	SV *	imp_datasv;
...
XS(XS_DBI__install_method)
{
	...
	char *	meth_name = (char *)SvPV(ST(1),na);
	char *	file = (char *)SvPV(ST(2),na);
	SV *	attribs;
...
XS(XS_DBI__debug_dispatch)
{
    dXSARGS;
    if (items < 1 || items > 3)
	croak("Usage: DBI::_debug_dispatch(sv, level=dbis->debug, file=Nullch)");
    {
	SV *	sv = ST(0);
	int	level;
	char *	file;
...
XS(XS_DBI__debug_handle)
{
    dXSARGS;
    if (items < 1 || items > 2)
	croak("Usage: DBI::_debug_handle(sv, level=0)");
    {
	SV *	sv = ST(0);
	int	level;
	int	RETVAL;
...
XS(XS_DBD_____st_bind_columns)
{
    dXSARGS;
    if (items < 2)
	croak("Usage: DBD::_::st::bind_columns(sth, attribs, ...)");
    {
	SV *	sth = ST(0);
	SV *	attribs = ST(1);
    D_imp_sth(sth);
...
XS(XS_DBD_____st_fetchrow)
{
    dXSARGS;
    if (items != 1)
	croak("Usage: DBD::_::st::fetchrow(sth)");
    SP -= items;
    {
	SV *	sth = ST(0);
    SV *retsv;
...
XS(XS_DBD_____st_fetch)
{
    dXSARGS;
    if (items != 1)
	croak("Usage: DBD::_::st::fetch(sth)");
    {
	SV *	sth = ST(0);
    int num_fields;
    if (CvDEPTH(cv) == 99)
...
XS(boot_DBI)
{
    ...
    XS_VERSION_BOOTCHECK ;
        ...
        newXS("DBI::_debug_dispatch", XS_DBI__debug_dispatch, file);
        newXS("DBI::_debug_handle", XS_DBI__debug_handle, file);
        ...
        newXS("DBD::_::st::fetchrow", XS_DBD_____st_fetchrow, file);
        newXS("DBD::_::st::fetch", XS_DBD_____st_fetch, file);
