/* $Id: DBI.xs,v 1.48 1995/10/24 00:24:53 timbo Rel $
 *
...
static char *
neatsvpv(sv)	/* return a tidy ascii value, for debugging only */
    SV * sv;
{
    STRLEN len;
    SV *nsv = NULL;
	...
	sv_catpvn(nsv, v, (STRLEN)dbis->debugpvlen);
	sv_catpv( nsv, "...");
    }else{
	sv_catpvn(nsv, v, len);
	sv_catpv( nsv, "'");
    ...
    SV *imp_datasv;
{
	...
	fprintf(DBILOGFP,"    dbih_setup_handle(%s=>%s, %s, %s)\n",
	    SvPV(orv,na), SvPV(h,na), imp_class, neatsvpv(imp_datasv));
    ...
    SV *attribs;
{
	...
	croak("Not a reference for bind_col(%s, %s, %s,...)",
		neatsvpv(sth), neatsvpv(col), neatsvpv(ref));
    ...
    SV *valuesv;
{
	...
	fprintf(DBILOGFP,"    STORE %s %s => %s\n",
		SvPV(h,na), neatsvpv(keysv), neatsvpv(valuesv));
    ...
    SV *a1, *a2;
{
    ...
    AV *handlers_av;
    I32 handlers = 0;
    SV  *evtype_sv = sv_2mortal(newSVpv(evtype,0));

    if (dbis->debug)
	fprintf(DBILOGFP,"NEW EVENT '%s', %s, %s",
			    evtype, neatsvpv(a1), neatsvpv(a2));

    handlers_av = (AV*)DBIc_HANDLERS(imp_xxh);
    if (SvTYPE(handlers_av) != SVt_PVAV) {
	warn("%s->{Handlers} (%s) is not an array reference",
	    neatsvpv(DBIc_MY_H(imp_xxh)), neatsvpv(handlers_av));
	/* drop through to default handler */
    } else {
	int i = handlers = av_len(handlers_av) + 1;
	while(--i >= 0) {
	    SV *sv = *av_fetch(handlers_av, i, 1);
	    warn("EVENT '%s' handler #%d '%s' ignored", evtype, i, neatsvpv(sv));
	    /* call handler here */
	}
    }
    ...
    XPUSHs(hrv);
    XPUSHs(evtype_sv);
    if (SvOK(a2) || SvOK(a1)) { XPUSHs(a1); }
    if (SvOK(a2))             { XPUSHs(a2); }
    PUTBACK;

    /* XXX optimise to prefetch the GV for DBI::default_handler once	*/
    perl_call_pv("DBI::default_handler", G_SCALAR);	/* XXX G_EVAL ? */

    SPAGAIN;
    a1 = POPs;
    PUTBACK;
    if (dbis->debug >= 3)
	fprintf(DBILOGFP, "DBI::default_handler returned %s", neatsvpv(a1));
...
XS(XS_DBI_dispatch)         /* prototype must match XS produced code */
{
    ...
    */
	...
	if (debug >= 2) {
	    ...
	    for(i=1; i<items; ++i)
		fprintf(DBILOGFP," %s", neatsvpv(ST(i)));
	    fprintf(DBILOGFP, ")\n");
	}
	...
	if (isGV(imp_msv) && CvXSUB(GvCV(imp_msv))) {	/* SHORT-CUT ALERT! */
...
void
set_err(sv, errval, errstr=&sv_undef)
    SV *	sv
    SV *	errval
    SV *	errstr
    CODE:
...
FETCH(sv)
    SV *	sv
    CODE:
    /* ### TIESCALAR CODE SEEMS TO BE BUST IN Perl5.000 ### */
	...
	SvROK_on(dbi_last_h);
	ST(0) = sv_mortalcopy(dbi_last_h);
	if (dbis->debug >= 2)
	    fprintf(DBILOGFP,"   $DBI::%s = %s (inner)\n",
				meth, SvPV(dbi_last_h,na));
	SvROK_off(dbi_last_h);
	XSRETURN(1);
    }
